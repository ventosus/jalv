...
make_combo(ControlID* record, float value, bool writable)
{
	...
	gtk_widget_set_sensitive(list_store, writable);
...
build_control_widget(Jalv* jalv, GtkWidget* window)
{
	...
	/* Add controllers for writable/readable properties (event-based controls) */
	const LilvNode* patch_rw []  = { patch_writable, patch_readable };
	const bool writable []       = { true, false };
	for (int i = 0; i < 2; ++i) {
		...
		LILV_FOREACH(nodes, p, properties) {
			...
			if (!record->value_type) {
				...
				controller = make_entry(record, writable[i]);
			} else if (record->value_type == jalv->forge.Path) {
				controller = make_file_chooser(record, writable[i]);
			} else {
				controller = make_controller(record, get_float(record->def, 0.0f), writable[i]);
			}
